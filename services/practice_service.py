            ...
            with sqlite3.connect(self.db_path) as conn:
                ...
                cursor.execute('''
                    SELECT practice_id, timestamp, selected_text, questions,
                           user_answers, evaluation_result, status
                    FROM practice_sessions
                ...
                if row:
                    practice_id, timestamp, selected_text, questions, user_answers, evaluation_result, status = row
                ...
                if existing:
                    ...
                    cursor.execute('''
                        ...
                        SET selected_text = ?, questions = ?, user_answers = ?,
                            evaluation_result = ?, status = ?, updated_at = CURRENT_TIMESTAMP
                        WHERE practice_id = ?
                    ''', (
                    ...
                    cursor.execute('''
                        ...
                        (practice_id, timestamp, selected_text, questions, user_answers, evaluation_result, status)
                        VALUES (?, ?, ?, ?, ?, ?, ?)
                    ''', (
                        practice_id,
